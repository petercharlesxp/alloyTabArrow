...
Ti.API.info("currentPhoto: " + JSON.stringify(currentPhoto));
	...
	if (OS_ANDROID) {
		...
		$.getView().activity.onCreateOptionsMenu = function(_event) {
			...
			mItemSave.addEventListener("click", function(_event) {
				...
				Ti.API.info("_event.source.id: " + JSON.stringify(_event.source.id));
			});
...
function handleButtonClicked(_event) {
	...
	Ti.API.info("returnParams: " + JSON.stringify(returnParams));
}
